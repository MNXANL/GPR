...
void PointCloud::setBorderColours(const std::vector< bool > &isBorderPoint, ShaderProgram &program)
{
	glm::vec4 red = glm::vec4(1, 0, 0, 1);
	if (isBorderPoint.size() != colors.size()) 
		return;
	
	for(unsigned int i=0; i < isBorderPoint.size(); i++)
	{
		if ( isBorderPoint[i] )
			colors[i] = red;
	}
